...
def secure_admin_required(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        # Tinitiyak na ang kasalukuyang session key ay tumutugma sa huling naitalang valid token ng system
...
def get_validated_client_ip():
    # Kukuha lamang ng IP sa trusted upstream headers ng Railway proxy host
    if request.headers.get('X-Forwarded-For'):
        ip = request.headers.get('X-Forwarded-For').split(',')[0].strip()
    else:
        ip = request.remote_addr
        
    # Simple sanitization validation laban sa Log Injection
...
def generate_secure_frames(validated_token):
    ...
    try:
        while camera_manager.is_running:
            # Control 2: Token Validation validation check bawat loop iteration
            ...
            if not success:
                retry_count += 1
                time.sleep(0.5 * retry_count) # Exponential structural pause
                if retry_count > max_retries:
                    break
                continue
            
            retry_count = 0 # Reset retry count pagkatapos ng successful read

            # Control 8: Non-Empty Frame Validation Processing
            if frame is None or frame.size == 0:
                continue

            # Control 8: Advanced Integrity check (Detect entirely corrupted/black dummy inputs)
            if cv2.mean(frame)[0] < 2.0: # Threshold for malicious frame freezing/blackouts
                continue

            # Control 9: Cryptographic Timestamp Overlaying (On-Screen Display Server Watermark)
            server_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")[:-3]
            cv2.putText(frame, f"LIVE SERVER TIME: {server_time}", (10, 30),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2, cv2.LINE_AA)

            # I-encode ang inayos na live frame
            ...
            # Control 6: Frame Rate Overload Throttling - Dynamic sleep calculations
    ...
    # Control 17: Fail-Closed Error Boundary Pattern
        ...
        # Tahasang isasara ang execution logic nang walang ipinapakitang error logs sa client side
        pass
    finally:
        with camera_manager._lock:
...
def request_stream_token():
    """
    Control 2: Endpoint na gumagawa ng cryptographic transaction token 
    bago payagang tawagin ang video feed URL path.
    """
...
def secure_video_feed():
    ...
    token_param: str = 'development_mode'
    camera_manager.active_token = token_param

    client_ip = get_validated_client_ip()

    # Control 11: Append-Only Audit Logging Initialization via database rules
    log = CameraLog(event='SECURE_STREAM_STARTED', ip_address=client_ip)
    db.session.add(log)
    db.session.commit()

    # Control 4: Initialize structural instantiation ng camera
    with camera_manager._lock:
    ...
    # Control 11: Aggressive Browser Cache Disabling Implementation Headers
    response.headers["Cache-Control"] = "no-cache, no-store, must-revalidate, private"
    response.headers["Pragma"] = "no-cache"
    response.headers["Expires"] = "0"

    # Control 14: Content Security Policy (CSP) & Clickjacking Restrictions Headers
